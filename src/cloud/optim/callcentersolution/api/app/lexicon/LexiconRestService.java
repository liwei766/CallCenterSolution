...
import cloud.optim.callcentersolution.api.app.lexicon.LexiconResponse.BulkResult;
import cloud.optim.callcentersolution.api.recaius.Lexicon;
import cloud.optim.callcentersolution.api.recaius.LexiconError;
import cloud.optim.callcentersolution.api.recaius.LexiconErrorCode;
import cloud.optim.callcentersolution.api.recaius.result.ErrorResult;
import cloud.optim.callcentersolution.api.recaius.result.LexiconGetResult;
import cloud.optim.callcentersolution.api.recaius.result.LexiconUpdateErrorDetailResult;
import cloud.optim.callcentersolution.api.recaius.result.LexiconUpdateSuccessResult;
import cloud.optim.callcentersolution.api.recaius.result.RecaiusResult;
...
public class LexiconRestService
{
	...
	public LexiconResponse importCsv(@Context HttpServletRequest req) {
		...
		try {
			...
			// ユーザ情報を取得する
			CustomUser customUser = loginUtility.getCustomUser();
			String companyId = customUser.getCompanyId();//kuromojiで更新した辞書情報でインスタンスを再生成するため
			Integer modelId = customUser.getRecaiusModelId();
			String serviceId = customUser.getRecaiusServiceId();
			String password = customUser.getRecaiusPassword();
			...
			if (res.getBulkResultList().isEmpty()) {
				...
				// ----- ユーザ辞書登録
				RecaiusResult result = speechService.updateLexicon(token, modelId, lexicons);
