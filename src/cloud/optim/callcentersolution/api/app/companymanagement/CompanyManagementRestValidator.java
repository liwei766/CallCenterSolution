...
import cloud.optim.callcentersolution.api.app.companymanagement.CompanyManagementRequest.EditForm;
...
import cloud.optim.callcentersolution.core.common.utility.Hash;
...
class CompanyManagementRestValidator {
  ...
  public void validateForPut(CompanyManagementRequest req) throws SQLException {
    ...
    // ----- 企業 ID
    {
      ...
      if (companyManagementService.isDuplicateCompanyId(value)) {
        throw new RestException(new RestResult(ResponseCode.COMPANY_MANAGEMENT_DUPLICATE_COMPANY_ID,
            null, "#companyManagement.companyId"));
      }
  ...
  public void validateForUpdate(CompanyManagementRequest req) throws SQLException {
    ...
    // 存在チェック
    CompanyManagement prevCompanyManagement =
        companyManagementService.getCompanyManagement(pkValue);
  ...
  /**
   * お問い合わせ画面URL発行の入力チェックと入力内容の補完.
   *
   * @param req 入力内容
   *
   * @throws SQLException 例外発生時
   */
  public void validateForIssueUrl(CompanyManagementRequest req) throws SQLException {
    String name = "#request";
    if (req == null) {
      throw new RestException(new RestResult(ResponseCode.INPUT_ERROR_REQUIRED, null, name));
    }
    ...
    // ----- 企業 ID
    companyManagement.setCompanyId(prevCompanyManagement.getCompanyId());

    // ----- 企業名
    companyManagement.setCompanyName(prevCompanyManagement.getCompanyName());

    // ----- 代理店企業ID
    companyManagement.setAgencyCompanyId(prevCompanyManagement.getAgencyCompanyId());

    // ----- リカイアスライセンスID
    companyManagement.setRecaiusLicenseId(prevCompanyManagement.getRecaiusLicenseId());

    // ----- リカイアスモデルID
    companyManagement.setRecaiusModelId(prevCompanyManagement.getRecaiusModelId());

    // ----- 音声判断レベル閾値
    companyManagement.setEnergyThreshold(prevCompanyManagement.getEnergyThreshold());

    // ----- 音声保存設定
    companyManagement.setSaveVoice(prevCompanyManagement.getSaveVoice());

    // ----- 接続元制限IPアドレス
    companyManagement.setPermitIpAddress(prevCompanyManagement.getPermitIpAddress());
    ...
    final String hashedCompanyId = this.createHashedCompanyId(companyManagement.getCompanyId());
    // TODO ↓uniq key が効いてるから不要？
    if (this.companyManagementService.isDuplicateHashedCompanyId(hashedCompanyId)) {
      throw new RestException(new RestResult(
          ResponseCode.INQUIRY_FORM_DUPLICATE_HASHED_COMPANY_ID,
          null, "#inquiryForm.issueUrl.hashedCompanyId"));
    }
    companyManagement.setHashedCompanyId(hashedCompanyId);
  }

  /**
   * お問い合わせ画面URL削除の入力チェックと入力内容の補完.
   *
   * @param req 入力内容
   *
   * @throws SQLException 例外発生時
   */
  public void validateForDeleteUrl(CompanyManagementRequest req) throws SQLException {
    String name = "#request";
    if (req == null) {
      throw new RestException(new RestResult(ResponseCode.INPUT_ERROR_REQUIRED, null, name));
    }
    ...
    companyManagement.setHashedCompanyId("");
  }
  ...
  public boolean validateAcgencyId(final CompanyManagement companyManagement) {
    ...
    CustomUser customUser = loginUtility.getCustomUser();
    if (!authUtil.isSysAdmin()
        && !customUser.getCompanyId().equals(companyManagement.getAgencyCompanyId())) {
      return false;
    }
    //*/
    return true;
   ...
   * @return 企業IDハッシュ
   */
  private final String createHashedCompanyId(final String companyId) {
    final String salt = Hash.generateSecureRandom().toString();
    StringBuilder sb = new StringBuilder(salt);
    sb.append(companyId);
    return Hash.getMessageDigestString(sb.toString());
  }
  ...
  private void checkPermitIpAddress(String permitIpAddress) {
    ...
    for (String each : rowData) {
      ...
      if (++rowCount > permitIpAddressMaxCount) {
        throw new RestException(new RestResult(
          ResponseCode.COMPANY_MANAGEMENT_PERMIT_IP_ADDRESS_MAX_COUNT_ERROR,
