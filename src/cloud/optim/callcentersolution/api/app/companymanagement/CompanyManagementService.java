...
public class CompanyManagementService {
   ...
   * @return 企業ID（存在しない場合はnull）
   */
  public String getCompanyIdByHash(final String hashedCompanyId) {
    CompanyManagement condition = new CompanyManagement();
    condition.setHashedCompanyId(hashedCompanyId);
    List<CompanyManagement> companyManagementList = companyManagementDao.findByExample(condition);
    if (companyManagementList != null && !companyManagementList.isEmpty()) {
      return companyManagementList.get(0).getCompanyId();
    }
    return null;
  }
  ...
  /**
   * 企業IDハッシュ重複チェック.
   * <p>同じ企業IDハッシュは登録できない.</p>
   * @param hashedCompanyId 企業IDハッシュ
   * @return 重複していたら ture
   */
  // TODO uniq key が効いてるから不要？
  public boolean isDuplicateHashedCompanyId(final String hashedCompanyId) {
    CompanyManagement condition = new CompanyManagement();
    condition.setHashedCompanyId(hashedCompanyId);
    List<CompanyManagement> companyManagementList = companyManagementDao.findByExample(condition);
    if (companyManagementList != null && !companyManagementList.isEmpty()) {
