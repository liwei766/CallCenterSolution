...
$( function () {
	...
	var data = {
		...
		component : {
			...
			urlIssueArea : { // ----- URL発行エリア

				selector : "#urlIssueArea",
			},

			urlIssueForm : { // ----- URL発行フォーム

				selector : "#urlIssueForm",
			},

			inquiryUrl : { // ----- お問い合わせURL

				selector : "#inquiryUrl",
			},

			inquiryIssue : { // ----- お問い合わせURL発行

				selector : "#inquiryIssue",
				handler : [
					[ "click", inquiryIssue ],
				],
			},

			inquiryDel : { // ----- お問い合わせURL削除
	...
	function _clearForm() {
		...
		data.component.urlIssueArea.$element.find( "form" )[0].reset();
	...
	function _disableForm() {
		...
		data.component.urlIssueArea.$element.find( "form *").prop( "disabled", true );
	...
	function _enableForm() {
		...
		data.component.urlIssueArea.$element.find( "form *").prop( "disabled", false );
	...
	function _initRegist() {
		...
		// URL発行エリア 非表示
		data.component.urlIssueArea.$element.prop( "hidden", true );
	...
	function _initEdit() {
		...
		// URL発行エリア 表示
		data.component.urlIssueArea.$element.prop( "hidden", false );
	...
	function _getSuccess( response, status, xhr, option ) {
		...
		var url = response.editResult.inquiryFormUrl;
		data.component.inquiryUrl.$element.val( url );
	 ...
	 * @param {Event} event イベント
	 */
	function inquiryIssue( event ) {

		_view.confirmDialog( _prop.getMessage( "companyManagement.confirm.issueUrl" ), { ok : _inquiryIssueOk } );
	 ...
	 * @param {Event} event イベント
	 */
	function _inquiryIssueOk( event ) {

		var form = { "editForm" : { 
						"companyManagement" : {
							"companyManagementId" : data.company.companyManagementId,
							"updateDate": data.company.updateDate
						}
					}
			};

		var url = _prop.getApiMap( "companyManagement.issueUrl" );

		var json = JSON.stringify( form );

		var option = {

			handleError : _inquiryIssueError,
			handleSuccess : _inquiryIssueSuccess
		};
	 ...
	 * @param {Object} API 呼び出し時に使用した option オブジェクト
	 */
	function _inquiryIssueError( xhr, status, errorThrown, option ) {

		var msgList = [ _prop.getMessage( "companyManagement.error.issueUrlError" ) ];
	 ...
	 * @param {Object} API 呼び出し時に使用した option オブジェクト
	 */
	function _inquiryIssueSuccess( response, status, xhr, option ) {

		if ( ! option.result.ok ) {

			_inquiryIssueError( xhr, status, null, option );
			return;
		}
		...
		if ( company.saveVoice) {
			...
			data.component.saveVoice.$element.prop("checked", true);

		} else {

			data.component.saveVoice.$element.prop("checked", false);
		}
		...
		_view.infoDialog( _prop.getMessage( "companyManagement.complete.issueUrl" ) );
	 ...
	 * @param {Event} event イベント
	 */
	function inquiryDel( event ) {

		_view.confirmDialog( _prop.getMessage( "companyManagement.confirm.delUrl" ), { ok : _inquiryDelOk } );
	}
	...
	function _inquiryDelOk( event ) {
		...
		var url = _prop.getApiMap( "companyManagement.deleteUrl" );

		var json = JSON.stringify( form );

		var option = {

			handleError : _inquiryDelError,
			handleSuccess : _inquiryDelSuccess
		};
	 ...
	 * @param {Object} API 呼び出し時に使用した option オブジェクト
	 */
	function _inquiryDelError( xhr, status, errorThrown, option ) {

		var msgList = [ _prop.getMessage( "companyManagement.error.delUrlError" ) ];
	 ...
	 * @param {Object} API 呼び出し時に使用した option オブジェクト
	 */
	function _inquiryDelSuccess( response, status, xhr, option ) {

		if ( ! option.result.ok ) {

			_inquiryDelError( xhr, status, null, option );
			return;
		}
		...
		_view.infoDialog( _prop.getMessage( "companyManagement.complete.delUrl" ) );
