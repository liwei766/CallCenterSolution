...
$( function () {
	...
	function csvImport( event ) {
		...
		if ( _util.isUndefined( fileList ) || fileList.length < 1 ) {
			...
			_view.errorDialog( _prop.getMessage( "csvimport.error.noFile" ) );
			return false;
		}
		...
		if ( _util.isUndefined( file ) || file.size < 1 ) {
			...
			_view.errorDialog( _prop.getMessage( "csvimport.error.emptyFile" ) );
			return false;
		}

		_view.confirmDialog( _prop.getMessage( "csvimport.confirm" ), {
			ok : _importOk,
			fileList : fileList,
			file : file,
		});
	...
	function _importOk( event ) {
		...
		var url = _prop.getApiMap( "knowledge.csvimport" );
...
//		return true;
	...
	function _importError( xhr, status, errorThrown, option ) {
		...
		var msgList = [ _prop.getMessage( "csvimport.error.importError" ) ];
...
//		}, 150 );	// ダイアログの非表示に最大 150ms かかるのでかぶらないように待つ
	...
	function _importSuccess( response, status, xhr, option ) {
		...
		if ( response.resultList[0].code === _prop.getProperty( "common.apiResponse.success" ) ) {
			...
			msg = _util.format( _prop.getMessage( "csvimport.complete" ), response.dataCount );
		...
		msg = _util.format( _prop.getMessage( "csvimport.error.partialError" ), response.errorCount );
		...
		if ( msgList.length < response.errorCount ) {
			...
			msg = _util.format(
				_prop.getMessage( "csvimport.error.tooManyError" ), msgList.length );
	...
	function csvExport( event ) {
		...
		_view.confirmDialog( _prop.getMessage( "csvimport.exportConfirm" ), { ok : _download } );
	...
	function _download( event ) {
		// ダウンロード
		_api.download( _prop.getApiMap( "knowledge.csvexport" ));
...
function downloadError () {
	var _prop = CCS.prop;
	var _view = CCS.view;
	_view.errorDialog(_prop.getMessage( "csvimport.error.exportError"));
}