...
//			},
	 ...
	 * @returns
	 */
	function lexiconUpdate( event ) {

		_view.confirmDialog( _prop.getMessage( "lexicon.confirm" ), { ok : _lexiconUpdateOk } );
	}
	...
	function exportFile( event ) {
		...
		_view.confirmDialog( _prop.getMessage( "lexicon.exportConfirm" ), { ok : downloadCSV } );
	 ...
	 * ユーザ辞書更新
	 */
	function _lexiconUpdateOk( event ) {
		...
		if ( _util.isUndefined( fileList ) || fileList.length < 1 ) {
			...
			_view.errorDialog( _prop.getMessage( "lexicon.error.noFile" ) );
			return false;
		}
		...
		if ( _util.isUndefined( file ) || file.size < 1 ) {
			...
			_view.errorDialog( _prop.getMessage( "lexicon.error.emptyFile" ) );
			return false;
		}
		...
		var url = _prop.getApiMap( "lexicon.update" ) ;
	...
	function _updateError( xhr, status, errorThrown, option ) {
		...
		var msgList = [ _prop.getMessage( "lexicon.error.updateError" ) ];
	...
	function _updateSuccess( response, status, xhr, option ) {
		...
		if ( response.resultList[0].code === _prop.getProperty( "common.apiResponse.success" ) ) {
			...
			msg = _util.format( _prop.getMessage( "lexicon.complete" ), response.dataCount );
		...
		msg = _util.format( _prop.getMessage( "lexicon.error.partialError" ), response.errorCount );
		...
		if ( msgList.length < response.errorCount ) {
			...
			msg = _util.format(
				_prop.getMessage( "lexicon.error.tooManyError" ), msgList.length );
	...
	function downloadCSV(event) {
		...
		let ext="/csv";
		let fileId="/USER_DICTIONARY";
		let apiURL = _prop.getApiMap( "lexicon.file" ) + ext + fileId;
...
function downloadError () {
	var _prop = CCS.prop;
	var _view = CCS.view;
	_view.errorDialog(_prop.getMessage( "lexicon.error.exportError"));
}
