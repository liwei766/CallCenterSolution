...
$( function () {
	...
	function _nextPage() {
		...
		if ( sendPageNo > data.totalPage ) {

			sendPageNo = data.totalPage;
		}
	 ...
	 * 検索.
	 *
	 * @param {String} sendPageNo
	 */
	function _search( sendPageNo ) {
	...
	function _searchSuccess( response, status, xhr, option ) {
		...
		if ( response.resultList[0].code === _prop.getProperty( "common.apiResponse.bizNoAuthError" ) ) {
			...
			return;
		} else if ( response.resultList[0].code === _prop.getProperty( "common.apiResponse.bizLimitAuthError" ) ) {
			// biz認証期限切れ
			_authErrorDialog( _prop.getMessage( "password.error.limitAuthError" ), { btn1 : _doAuth, btn2 : _redirectPage});
			return;
//		} else if ( response.resultList[0].code === _prop.getProperty( "common.apiResponse.bizSearchError" ) ) {
//			// biz検索エラー
//			_searchError( xhr, status, null, option );
//			return;
		}
		...
		$.each( response.searchResultList, function ( i, val ) {
			...
			var $id = $( "<th></th>" )
				.addClass( "ccs-password-user-listId" )
				.attr( "data-ccs-id", id )
				.text( id )
				.appendTo( $tr );
		...
		var pageInfo = response.searchResultList.pageInfo;
	
		data.pageNo = pageInfo.pageNo;

		var pageDisp = "0";
		if ( pageInfo.offset > 0) {
			pageDisp = pageInfo.offset + " - " + ( pageInfo.offset + ( pageInfo.pageSize - 1 ) );
		}
		data.component.page.$element.text( pageDisp );
		data.component.totalNumber.$element.text( pageInfo.totalNumber );
	...
	function _updateSuccess( response, status, xhr, option ) {
		...
		if ( response.resultList[0].code === _prop.getProperty( "common.apiResponse.bizNoAuthError" ) ) {
			...
			_authErrorDialog( _prop.getMessage( "password.error.limitAuthError" ), { btn1 : _doAuth, btn2 : _redirectPage});
			return;
//		} else if ( response.resultList[0].code === _prop.getProperty( "common.apiResponse.bizUpdateError" ) ) {
//			// biz更新エラー
//			_updateError( xhr, status, null, option );
//			return;
		} 
	...
	function _authSuccess( response, status, xhr, option ) {
		...
		if ( response.resultList[0].code === _prop.getProperty( "common.apiResponse.bizFailAuthError" ) ) {

			message = _prop.getMessage( "password.error.failAuthError" );
		}
		...
		if ( message ) {
			...
			if ( $dialog.hasClass( "show" ) ) {
				...
				_authDialog( message, { btn1 : _doAuth , btn2 : _redirectPage});
				return;
			}
